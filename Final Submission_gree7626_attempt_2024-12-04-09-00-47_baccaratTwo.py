...
def compute_score(hand): #compute the score of the hand
    total_value = sum(VALUES[CARDS.index(card)] for card in hand)
    return total_value % 10

def get_bet_amount(total_money): #Get a bet amount from the user
    while True:
        try:
...
def get_user_role():#Ask user for the role
    while True:
...
def draw_card():#Draw a random Card
    return random.choice(CARDS)

def display_hand_and_score(hand, role):#Display Hand and role for the score
    score = compute_score(hand)
    print(f"\n{role.capitalize()}'s cards: {hand}")
    print(f"{role.capitalize()}'s score: {score}")
    return score

def check_natural_win(player_score, banker_score): #Check for Natural Win
...
def ask_to_draw_card(role): #Ask user if they want to draw a card
    while True:
...
def update_total_money(total_money, bet_amount, result, user_is_player): # update players money if won
    print(result)
...
def play_round(total_money): #Main single round of play
